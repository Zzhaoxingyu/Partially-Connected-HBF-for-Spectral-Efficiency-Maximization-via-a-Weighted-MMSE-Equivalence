function [rate,V_B,V_RF,W_B,W_RF] = MMSE_EI_wbmethod(Nt,Nr,Nt_rf,Nr_rf,Vn,Ns,H,Nk,frf_1,wrf_1)
...
while (iter<=20 && trigger>1e-4) 
    ...
    %precoding
    [V_RF] = MMSE_EI_Frf_algorithm(V_RF, w, H1,M,Nk,Vn,Nt_rf,Ns);
    for k = 1 : Nk    
       V_U(:,:,k) =(V_RF'*H1(:,:,k) * H1(:,:,k)'* V_RF+  Vn * w(k) *(V_RF)'*V_RF)^(-1)*V_RF'*H1(:,:,k);
       V_equal(:,:,k) = V_RF * V_U(:,:,k);
       v(k) = trace(V_equal(:,:,k)' * V_equal(:,:,k));
       H2(:,:,k) = H(:,:,k) * V_equal(:,:,k);
    end
    %combining
     [W_RF] = MMSE_EI_Wrf_algorithm(W_RF, v, H2,N,Nk,Vn,Nr_rf,Ns);
     for k = 1:Nk
         W_B(:,:,k) =(W_RF'*H2(:,:,k) * H2(:,:,k)'* W_RF+  Vn * v(k)*(W_RF)'*W_RF)^(-1)*W_RF'*H2(:,:,k);
